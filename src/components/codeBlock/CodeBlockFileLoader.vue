<script setup lang="ts" name="CodeBlockFileLoader">
	...
	async function loadCode() {
		try {
			...
			if (!props.lang) detectedLang.value = props.lang
	...
	const lineDigits = computed(() => String(lines.value.length).length)
			...
			<table class="code-table">
				<tbody>
					<tr v-for="(line, index) in lines" :key="index">
						<!-- 行號 -->
						<td class="gutter" :style="{ width: lineDigits + 3 + 'ch' }">{{ index + 1 }}</td>
						<!-- 程式碼 -->
						<td class="code-cell">
